import {collection, addDoc, updateDoc, doc, getDoc, getDocs, query, where, arrayUnion, Timestamp , deleteDoc } from 'firebase/firestore';
...
export const createChat = async (userId, title ,summary , sourceId) => {
...
export const checkExistingChat = async (userId, title) => {
  const q = query(collection(db, 'chats'), where('userId', '==', userId), where('title', '==', title));
  const querySnapshot = await getDocs(q);
